...
about: ' Tanstack Form is a best tool for handling form in our React Apps which provides Reactive, Complex validation, Accessibility and custom styling. Which help to build a better and robust form in our Apps.'
tags:
  [
...
Tanstack Form provides `createFormFactory` function which let us create form instance. We can create form schema and validation for server using `onServerValidate` function.
...
Here witch `action` function we are validating the form data and doing the server side action. We are using `formFactory.validateFormData` function to validate the form data.
...
<Summary>
  <LI>
    `useFormState` is hook provided by React which lets us work with server
    actions forms. It allows us to update state based on the result of a form
    action.
  </LI>
  <LI>
    We are using `formFactory.useForm` hook to get the form instance.
    `useTransform` function to merge the form state with the server state.
  </LI>
  <LI>
