...
> Note: We will use Docker as driver for Minikube as it is the most stable one and easy to start. so make sure your Docker is running.
...
Vist [here](https://kubernetes.io/docs/tasks/tools/) to install Kubectl on different platforms.
...
> Note: We are using same app which we have dockerized in our pervious article for development.

#### Creating a Dockerfile for Frontend

> Note: Rename the existing Dockerfile to Dockerfile.dev

Create a new file called Dockerfile in the frontend directory and add the following code to it.
...
> Note: Make sure you are in the root directory of the project. out of frontend and backend directory.

#### YAML file for Deployment

Create a new file called `Deployment.yaml` and add the following code to it.
...
> Note : Make sure you have started your Minikube.
...
> Note : You can rebuild you images if you do changes in app with same name and load to minikube and then apply the changes.
