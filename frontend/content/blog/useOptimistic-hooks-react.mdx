...
useTransition hooks helps useOptimistic hooks to use in client components when we are using click events. Right now we have to wrap the useOptimistic setter function with a `useTransition` setter function.
